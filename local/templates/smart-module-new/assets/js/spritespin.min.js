! function ($) {
	...
	Spin.clamp = clamp, Spin.wrap = wrap, Spin.bind = bind, Spin.sourceArray = function (path, opts) {
		...
		for (i = lStart; i <= lEnd; i += 1)
			for (j = fStart; j <= fEnd; j += 1) temp = path.replace("{lane}", padNumber(i, digits, 0)), temp = temp.replace("{frame}", padNumber(j, digits, 0)), result.push(temp);
		return result
	...
	function ($) {
		...
		function bindOrientationEvent(callback) {
			unbindOrientationEvent(), $(window).bind(orientationEvent, callback)
		}
		var fn = function () {
			...
			], i = 0, l = fnMap.length, ret = {}; i < l; i++)
				if (val = fnMap[i], val && val[1] in document) {
					for (i = 0, valLength = val.length; i < valLength; i++) ret[fnMap[0][i]] = val[i];
					return ret
				} return !1
		}(),
			changeEvent = fn.fullscreenchange + "." + SpriteSpin.namespace + "-fullscreen",
			orientationEvent = "orientationchange." + SpriteSpin.namespace + "-fullscreen";
		SpriteSpin.extendApi({
			...
			requestFullscreen: function (opts) {
				...
				var api = this,
					...
					oSource = data.source,
					oSize = data.sizeMode,
					oResponsive = data.responsive,
					enter = function () {
						data.width = window.screen.width, data.height = window.screen.height, data.source = opts.source || oSource, data.sizeMode = opts.sizeMode || "fit", data.responsive = !1, SpriteSpin.boot(data)
					},
					exit = function () {
						data.width = oWidth, data.height = oHeight, data.source = oSource, data.sizeMode = oSize, data.responsive = oResponsive, SpriteSpin.boot(data)
					};
				bindChangeEvent(function () {
					isFullscreen() ? (enter(), bindOrientationEvent(enter)) : (unbindChangeEvent(), unbindOrientationEvent(), exit())
	...
	function ($, SpriteSpin) {
		...
		function end(e, data) {
			if (data.dragging) {
				...
				"horizontal" === data.orientation ? (dS = data.ndX, dF = data.ddX) : (dS = data.ndY, dF = data.ddY), dS > snap || dF > fling ? frame = data.frame - 1 : (dS < -snap || dF < -fling) && (frame = data.frame + 1), SpriteSpin.resetInput(data), SpriteSpin.updateFrame(data, frame, lane), SpriteSpin.stopAnimation(data)
			}
		}
