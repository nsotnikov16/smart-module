...
if(!function_exists('komboxShowField'))
{
	function komboxShowField($arItem)
	{
		switch($arItem['SETTINGS']['VIEW'])
		{
			...
			?>
				<div class="kombox-num kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<table>
						...
						<tr>
							<td colspan="2" class="kombox-range">
								<div  
			...
			case 'SELECT':
			?>
				<div class="kombox-select kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
			...
			case 'LIST':
			?>
				<div class="kombox-list kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<select 
					...
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						...
						<div class="lvl2<?echo $ar["DISABLED"]? ' kombox-disabled': ''?><?echo $ar["CHECKED"]? ' kombox-checked': ''?>">
							<input
								...
								name="<?echo $arItem["CODE_ALT"]?>" 
								id="<?echo $ar["CONTROL_ID"]?>"
								<?echo ($ar["DISABLED"] && !$ar["CHECKED"])? ' disabled="disabled"': ''?>
			...
			case 'RADIO':
			?>
				<div class="kombox-radio kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?komboxOtherValues($arItem, 'start');?>
						<div class="lvl2<?echo $ar["DISABLED"]? ' kombox-disabled': ''?><?echo $ar["CHECKED"]? ' kombox-checked': ''?>">
							<input
			...
			case 'LINK':
			?>
				<div class="kombox-link kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?komboxOtherValues($arItem, 'start');?>
						<div class="lvl2<?echo $ar["DISABLED"]? ' kombox-disabled': ''?><?echo $ar["CHECKED"]? ' kombox-checked': ''?>">
