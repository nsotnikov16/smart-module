...
<? if ($arResult["FAVORITES_CITIES"]): ?>
	<?/*<select onchange="$('#location_'+$(this).val()).trigger('click')">
		<? foreach ($arResult["FAVORITES_CITIES"] as $favKey => $favCityFields): ?>
			<option value="<?=$favKey?>" <?if($arResult["CURRENT_CITY"]["NAME"]==$favCityFields["FIELDS"]["NAME"]){?>selected<?}?>><?= $favCityFields["FIELDS"]["NAME"] ?></option>
		<? endforeach ?>
	</select>*/?>
    <select onchange="location.href=this.value">
        <? foreach ($arResult["FAVORITES_CITIES"] as $favKey => $favCityFields): ?>
            <option value="https://<?= $favKey!="default" ? $favKey : '' ?><?if($favKey!="default"):?>.<?endif;?>smart-module.ru/" <?if($arResult["CURRENT_CITY"]["NAME"]==$favCityFields["FIELDS"]["NAME"]){?>selected<?}?>><?= $favCityFields["FIELDS"]["NAME"] ?></option>
        <? endforeach ?>
    </select>
<? endif ?>
                ...
                <div class="wf-row">
                    <? if ($arResult["FAVORITES_CITIES"]): ?>
                        <ul class="list-unstyled wf-primary-cities" style="">
                            <? foreach ($arResult["FAVORITES_CITIES"] as $favKey => $favCityFields): ?>
                                <li class="pick-location-final" id="location_<?=$favKey?>"data-locid="<?= $favCityFields["FIELDS"]["ID"] ?>" data-url="<?=$favCityFields["URL"]. $_SERVER['REQUEST_URI']?>"><?= $favCityFields["FIELDS"]["NAME"] ?></li>
                            <? endforeach ?>
                        </ul>
                    <? endif ?>
                </div>
